...
if (pauseBtn) {
  ...
  pauseBtn.addEventListener('pointerdown', togglePause, { passive: false });
  pauseBtn.addEventListener('click', togglePause, { passive: false });
...
function stopFlap() {
  ...
  // Clear any pending continuous flap timeout
  if (typeof flapInt === 'number') {
    clearTimeout(flapInt);
  }
...
document.addEventListener('keydown', e => {
  ...
  if (gameOver) {
    // During game over, allow numeric level selection for restart
    const inputEl = document.getElementById('level-restart-input');
    if (inputEl) {
      if (e.key === 'Enter') {
        const v = parseInt(inputEl.value, 10);
        if (Number.isFinite(v)) startAgainAtLevel(v);
        return;
      }
      if (e.key === 'Backspace') {
        inputEl.value = String(inputEl.value).slice(0, -1);
        return;
      }
      if (/^\d$/.test(e.key)) {
        inputEl.value = (inputEl.value || '') + e.key;
        // If within range and one-digit scenario, user can press Enter; otherwise allow more digits
        return;
      }
    }
    return;
  ...
  switch (e.key) {
    ...
    case ' ':
      if (!spacePressed) {
        ...
        butterfly.classList.add('single-flap');
        wingsUp = true;
        singleFlap = setTimeout(() => {
          butterfly.style.transform = 'scale(1) rotate(0deg)';
          butterfly.classList.remove('single-flap');
          wingsUp = false;
        }, 300);
      }
      if (!flapInt) {
        // Start continuous flapping after a short delay to distinguish from single tap
        flapInt = setTimeout(() => {
          if (spacePressed) {
            butterfly.classList.remove('single-flap');
            butterfly.classList.add('flapping');
            flapInt = setInterval(() => {
              butterfly.style.transform = wingsUp ? 'scale(1) rotate(0deg)' : 'scale(1.1) rotate(-6deg)';
              butterfly.classList.toggle('flapping');
              wingsUp = !wingsUp;
            }, 150);
          }
        }, 250);
...
document.addEventListener('keyup', e => {
  if (e.key === ' ') {
    ...
    stopFlap();
    butterfly.style.transform = 'scale(1) rotate(0deg)';
    butterfly.classList.remove('flapping', 'single-flap');
    wingsUp = false;
  }
});
      ...
      if ((dx * dx + dy2 * dy2) < (hoopR * hoopR) && !gameOver && !superSlide.active && !invincibilityMode) {
        ...
        if (now - lastHitAt > HIT_COOLDOWN_MS) {
          ...
          if (lives <= 0 && !infiniteLives) {
            ...
            stopMusic();
            stopFlap();
            // Show enhanced game-over UI with level selection
            showLevelSelectOnGameOver();
...
function startGame() {
  ...
  butterfly.style.left = bx + 'px';
  butterfly.style.top = by + 'px';
  butterfly.classList.remove('flapping');
  wingsUp = false;
  ...
  // Show mobile pause button if applicable
  if (pauseBtn && ("ontouchstart" in window || navigator.maxTouchPoints > 0)) {
...
function setupPointerFlapControls() {
  ...
  const dismissIfVisible = (e) => {
    ...
    // When game over, do not auto-restart; wait for level selection
    if (gameOver && gameOverBox && !gameOverBox.hidden) { 
      const inputEl = document.getElementById('level-restart-input');
...
function showLevelSelectOnGameOver() {
  ...
  const lastLevel = Math.max(1, highestLevelAchieved);
  
  const html = `
    <div class="game-over-content">
      <div class="congratulations-section">
        <h2>🎉 Congratulations!</h2>
        <p class="level-reached">You reached <strong>Level ${level}</strong></p>
        <p class="restart-prompt">How would you like to play again?</p>
      </div>
      
      <div class="level-selection">
        <div class="two-options">
          <button id="restart-level-1" class="option-btn level-1-btn">
            <div class="option-icon">🆕</div>
            <div class="option-title">Start Fresh</div>
            <div class="option-desc">Begin from Level 1</div>
          </button>
          
          <button id="restart-last-level" class="option-btn level-last-btn">
            <div class="option-icon">🔄</div>
            <div class="option-title">Continue Challenge</div>
            <div class="option-desc">Start from Level ${lastLevel}</div>
          </button>
        </div>
      </div>
      
      <div id="leaderboard" class="leaderboard-section"></div>
    </div>
  `;
  
  gameOverBox.innerHTML = html;
  
  // Re-attach leaderboardBox reference
  ...
  document.getElementById('restart-level-1').addEventListener('click', () => {
    startAgainAtLevel(1);
  });
  
  document.getElementById('restart-last-level').addEventListener('click', () => {
    startAgainAtLevel(lastLevel);
  });
}
