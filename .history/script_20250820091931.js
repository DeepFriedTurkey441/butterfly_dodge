...
// Progression rules
const FLOWERS_PER_LIFE = 11; // gain 1 life when reaching 11 flowers, then score resets to 0
...
function checkFlowers() {
  flowers.forEach((f, i) => {
    if (f && isColliding(butterfly, f)) {
      score++;
      // Award an extra life when score reaches FLOWERS_PER_LIFE, then reset score to 0
      if (score >= FLOWERS_PER_LIFE) {
        lives += 1;
        score = 0;
      }

      // Level up when lives reaches threshold, then reset lives to 3
