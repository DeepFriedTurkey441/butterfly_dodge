...
function setPauseMessage() {
  if (!pauseBox) return;
  const mobile = shouldUseMobileFlow();
...
function enableTouchLocks() {
  ...
  // Explicitly show the left rail only during mobile sessions
  document.body.classList.add('show-rail');
}

function disableTouchLocks() {
  document.body.classList.remove('touch-lock');
  document.body.classList.remove('show-rail');
}
