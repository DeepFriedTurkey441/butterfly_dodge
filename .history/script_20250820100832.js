...
let muted = false;
const sfx = {
  flower: new Audio('data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAAACAAACcQAAACAA...'),
  hit: new Audio('data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAAACAAACcQAAACAA...'),
  level: new Audio('data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAAACAAACcQAAACAA...')
};
...
function checkFlowers() {
  flowers.forEach((f, i) => {
    if (f && isColliding(butterfly, f)) {
      ...
      // pop animation
      f.classList.add('pop');
      if (!muted) sfx.flower.currentTime = 0, sfx.flower.play();
    ...
    nets.forEach(n => {
      ...
      if (dx * dx + dy2 * dy2 < r * r && !gameOver) {
        ...
        if (now - lastHitAt > HIT_COOLDOWN_MS) {
          ...
          if (!muted) sfx.hit.currentTime = 0, sfx.hit.play();
          if (lives <= 0) {
            running = false;
            gameOver = true;
            gameOverBox.hidden = false;
