...
// Progression rules
const SCORE_PER_LIFE = 10; // gain 1 life every 10 points
...
function checkFlowers() {
  flowers.forEach((f, i) => {
    if (f && isColliding(butterfly, f)) {
      score++;
      // Award an extra life every SCORE_PER_LIFE points
      if (score % SCORE_PER_LIFE === 0) {
        lives = Math.min(MAX_LIVES_BEFORE_LEVEL, lives + 1);
      }
